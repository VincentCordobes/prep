...
let of_yojson interval =
  let value = interval |> member "value" |> to_int in
...
let to_yojson = function
  | Day value -> `Assoc [ ("value", `Int value); ("unit", `String "day") ]
  | Week value -> `Assoc [ ("value", `Int value); ("unit", `String "week") ]
...
let compare a b =
  let to_days = function Day n -> n | Week n -> n * 7 in
